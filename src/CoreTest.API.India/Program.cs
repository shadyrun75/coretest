...
using CoreTest.JsonConverters.India;
using common_logger;
...
try
{
    ...
    builder.Services.AddSingleton(new Setup()
    {
        ...
        {
            new ActionElement(1, CoreTest.Models.Enums.ActionType.SendCallback, CoreTest.Models.Enums.CaseType.BeforeCreateClient),            
            new ActionElement(1, CoreTest.Models.Enums.ActionType.SendCallback, CoreTest.Models.Enums.CaseType.AfterCreateClient),            
        }
    });
    builder.Services.AddScoped<INotificationCore, NotificationCore>();
    builder.Services.AddSingleton<IClientData>(new ClientData());
    builder.Services.AddScoped<IClientCore, CoreTest.Cores.India.ClientCore>();
